...
router.post("/add", async (req: Request, res: Response): Promise<void> => {
  try {
      ...
      if (error.code === 11000) {
          res.status(409).json({ message: "Duplicate sleep entry already exists." });
      }
...
router.get("/user/:userId/sleepLogs", async (req: Request, res: Response): Promise<void> => {
  try {
      const sleepEntries = await SleepEntry.find({ userId: req.params.userId }).sort({ startTime: -1 });

      // Remove invalid logs (negative duration)
      const validEntries = sleepEntries.filter((entry: any) => entry.endTime > entry.startTime);

      if (!validEntries.length) {
          res.status(404).json({ message: "No valid sleep records found" });
      }
...
router.get("/user/:userId/debt", async (req: Request, res: Response): Promise<void> => {
  try {
...
router.put("/user/:userId/sleepGoal", async (req: Request, res: Response): Promise<void> => {
    try {
...
router.get("/user/:userId/sleepGoal", async (req: Request, res: Response): Promise<void> => {
    try {
