...
export const getSleepLogs = async (userId: string) => {
    try {
        const res = await axios.get(`${API_URL}/user/${userId}/sleepLogs`);
...
export const getSleepGoal = async (userId: string) => {
    try {
        const res = await axios.get(`${API_URL}/user/${userId}/sleepGoal`);
...
export const updateSleepGoal = async (userId: string, sleepGoal: number) => {
    try {
        await axios.put(`${API_URL}/user/${userId}/sleepGoal`, { sleepGoal: sleepGoal * 60 }); // Convert to minutes
    } catch (error) {
